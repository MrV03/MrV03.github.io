var NodesJs = function (parameters) {
    ...
    t_NodesJs.particleSize = parameters.particleSize || 7;
    t_NodesJs.lineSize = parameters.lineSize || 1;
    t_NodesJs.particleColor = parameters.particleColor ? 'rgba(' + parameters.particleColor.join(',') + ')' : 'rgba(0, 0, 0, 0.3)';
    t_NodesJs.lineColor = parameters.lineColor || '255,255,255';
    ...
    t_NodesJs.number = parameters.number || 100;
    t_NodesJs.speed = parameters.speed || 20;
    t_NodesJs.nobg = parameters.nobg || false;
    t_NodesJs.pointerCircleRadius = parameters.pointerCircleRadius || 150;
    ...
    t_NodesJs.placeNodes = function (number) {
        ...
        for (var i = 0; i < number; i++) {
            t_NodesJs.nodes.push([
                Math.floor(Math.random() * (cw - 0 + 1)),
                Math.floor(Math.random() * (ch - 0 + 1)),
    ...
    t_NodesJs.pointerCircleRadius &&
        window.addEventListener('mousemove', function (event) {
            ...
            t_NodesJs.nodes.forEach(function (_node, _node_i) {
                ...
                if (isPositive(xsig) && isNegative(ysig)) {
                } else if (isNegative(xsig) && isNegative(ysig)) {
                    angle = ((Math.PI / 2) - angle) + (Math.PI / 2);
                } else if (isNegative(xsig) && isPositive(ysig)) {
                    angle = angle + Math.PI;
